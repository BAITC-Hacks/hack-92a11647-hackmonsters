...
| POST | `/api/agent/plan` | Агент с инструментами: подобрать и сравнить допустимые планы |
...
`city_agent.py` добавляет ограниченный цикл **модель → инструмент → результат →
следующий инструмент**, а не шаблонный текст, выдаваемый за AI.
В UI под картой эффекта появилась панель **«AI-агент: планировщик»**:

1. Выберите цель: городской Score, слабейший район или критические показатели.
2. Нажмите **«Предложить план с AI-агентом»**. Можно улучшать текущий допустимый
   план или начать с пустого выбора. Частично заполненный план сначала завершите
   либо сбросьте — агент не должен молча игнорировать ваш выбор.
3. Сравните бюджет, Score, критические показатели, районы и журнал инструментов.
4. Только кнопка **«Применить предложенный план»** меняет корзину. После неё
   обычный калькулятор повторно проверяет предложение. При смене плана, цели или
   провайдера старое предложение нельзя применить.

Пример HTTP-запроса: [examples/agent_request.json](examples/agent_request.json).
Для подбора с нуля передайте `{"objective":"city_score","provider":"auto"}`.
Ответ содержит `proposed_plan`, полный `simulation`, сравнение `comparison`
с текущим планом (или исходной базой), `provider_used`, безопасный журнал
`steps`, счётчики и `requires_confirmation: true`. Существующий `/api/analyze`
можно вызвать после применения, чтобы получить текстовое объяснение.

Инструменты модели:
- `get_catalog` — прочитать только серверные данные и правила;
- `simulate_plan` — проверить и рассчитать пять мер через CitySimulator;
- `improve_plan` — сравнить ограниченное соседство: замена одной меры либо
  перенос одной районной меры; возвращает до трёх лучших известных вариантов;
- `finish_plan` — выбрать уже проверенный candidate_id, не новые числа.

Цели ранжируются лексикографически: Score → меньше критических → слабейший D →
остаток бюджета; для слабейшего района — D → Score → меньше критических →
остаток; для критических — меньше критических → Score → D → остаток.
Таким образом предложение не хуже исходного допустимого плана по выбранной
цели среди изученных кандидатов; **глобальный оптимум не гарантируется**.
Другие показатели могут ухудшиться — их дельты показаны отдельно.
Сравнение считается на сервере; произвольный текст модели не выдаётся за расчёт.

Предельный бюджет одного запуска: шесть запросов модели (включая fallback),
шесть вызовов инструментов, шестьсот попыток расчёта кандидатов, двадцать пять
секунд на провайдера и шестьдесят секунд всего. Счётчики кандидатов не включают
валидацию исходного плана. Одновременно допускаются два запуска на процесс;
третий получает 429. После ошибок возвращается 502/503, а не фиктивный план.
Эти ограничения не заменяют авторизацию и rate limiting при публичном размещении.

OpenAI использует существующий `OPENAI_MODEL` (по умолчанию GPT-4o).
NVIDIA-модель должна поддерживать Chat Completions function calling; поддержка
JSON mode сама по себе недостаточна. Strict-схемы включаются для режима
`json_schema`; в остальных режимах остаётся обязательная локальная проверка.
Файлы, shell, сеть общего назначения и изменения БД модели не доступны.
Кеш кандидатов и история существуют только внутри одного запроса.

Реализация следует [OpenAI Docs: function calling](https://developers.openai.com/api/docs/guides/function-calling):
инструменты объявлены JSON-схемами, приложение выполняет разрешённую функцию
и возвращает результат через `tool_call_id`; параллельные вызовы выключены.
`tests/test_city_agent.py` проверяет реальный SDK на HTTP-моках без ключей:
полный цикл, точность DTO, невалидные/неизвестные инструменты, fallback,
лимиты, отмену, изоляцию запросов и невозможность завершить непроверенный план.
