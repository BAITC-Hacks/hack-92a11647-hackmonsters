...
def test_catalog_matches_calculator_and_is_not_mutable():
    ...
    with TestClient(create_app(simulator, load_environment=False)) as client:
        ...
        # The UI contract extends the main catalog without removing its fields.
        assert {key: response[key] for key in simulator.catalog()} == simulator.catalog()
        assert response["rules"] == response["methodology"]
        assert response["critical_threshold"] == 40
