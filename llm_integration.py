...
Сохраняй ограничения сценария: нельзя рекомендовать увеличить фиксированный
бюджет или число решений. Предлагай замену или перераспределение мер в рамках
лимитов калькулятора. Снижение штрафа описывай через его исходное и итоговое
значения; не называй отрицательную дельту штрафа отрицательным улучшением.
...
Это правило касается также порога критичности, сроков, версии схемы и чисел
в названиях. Нельзя копировать числовые литералы из текстовых полей JSON.
Порог цитируй ссылкой на critical_threshold или пиши «ниже критического порога».
Перед ответом проверь каждую строку: цифры допустимы лишь внутри корректного
токена из numeric_facts. Если подходящего токена нет, используй качественное
описание без числа. Пиши кратко: общая оценка — до двух предложений, в каждом
массиве — не более трёх коротких пунктов.
